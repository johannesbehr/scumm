...
GuiManager::GuiManager() : _redrawStatus(kRedrawDisabled), _stateIsSaved(false),
	...
	/*
	printf("GuiManager::GuiManager(1)\n");
	_theme = 0;
	_useStdCursor = false;
	...
	initTextToSpeech();
#endif // USE_TTS
	printf("GuiManager::GuiManager(2)\n");
	ConfMan.registerDefault("gui_theme", "scummremastered");
	Common::String themefile(ConfMan.get("gui_theme"));
	printf("GuiManager::GuiManager(3)\n");
	...
	printf("GuiManager::GuiManager(4)\n");
#ifdef __DS__
	...
	if (!loadNewTheme("builtin", gfxMode)) {
		...
		printf("Failed to load any GUI theme, aborting!\n");
	}
#else
	
	// Try to load the theme
	if (!loadNewTheme(themefile, gfxMode)) {
			printf("GuiManager::GuiManager(5)\n");
		// Loading the theme failed, try to load the built-in theme
		if (!loadNewTheme("builtin", gfxMode)) {
				printf("GuiManager::GuiManager(6)\n");
			// Loading the built-in theme failed as well. Bail out
			error("Failed to load any GUI theme, aborting");
		}
	}
...
bool GuiManager::loadNewTheme(Common::String id, ThemeEngine::GraphicsMode gfx, bool forced) {
	...
	printf("GuiManager::loadNewTheme(1)\n");
	...
	printf("GuiManager::loadNewTheme(2)\n");
	...
	assert(newTheme);
	printf("GuiManager::loadNewTheme(3)\n");

	if (!newTheme->init()){
		printf("GuiManager::loadNewTheme(4)\n");
		return false;
	}
	printf("GuiManager::loadNewTheme(5)\n");
