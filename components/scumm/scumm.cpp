...
ScummEngine::ScummEngine(OSystem *syst, const DetectorResult &dr)
	: Engine(syst),
	  ...
	  _game(dr.game),
	  _rnd("scumm"),
//	  _pauseDialog(0), _messageDialog(0), _versionDialog(0),
	...
	if (ConfMan.hasKey("nosubtitles")) {
		...
		printf("Configuration key 'nosubtitles' is deprecated. Converting to 'subtitles'\n");
...
void ScummEngine::waitForTimer(int msec_delay) {
	...
	while (!shouldQuit()) {
		...
		/*int mode = get_charset_mode();
		if(mode!=lastmode){
			printf("Charset mode changed: %d!!!\n", mode);
			lastmode = mode;
		}*/
