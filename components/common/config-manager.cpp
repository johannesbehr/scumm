...
int ConfigManager::getInt(const String &key, const String &domName) const {
	...
	if (value.c_str() == errpos)
		//error("ConfigManager::getInt(%s,%s): '%s' is not a valid integer",		      key.c_str(), domName.c_str(), errpos);
		printf("ConfigManager::getInt(%s,%s): '%s' is not a valid integer\n",		      key.c_str(), domName.c_str(), errpos);
...
bool ConfigManager::getBool(const String &key, const String &domName) const {
	...
	if (parseBool(value, val))
		return val;
	printf("ConfigManager::getBool(%s,%s): '%s' is not a valid bool\n", key.c_str(), domName.c_str(), value.c_str());
	//error("ConfigManager::getBool(%s,%s): '%s' is not a valid bool",key.c_str(), domName.c_str(), value.c_str());
	return 0;
}
