...
CineMetaEngine::CineMetaEngine() : AdvancedMetaEngine(Cine::gameDescriptions, sizeof(Cine::CINEGameDescription), cineGames, optionsList) {
	_singleId = "cine";
	_guiOptions = GUIO2(GUIO_NOSPEECH, GAMEOPTION_ORIGINAL_SAVELOAD);
}
	
PlainGameDescriptor CineMetaEngine::findGame(const char *gameId) const {
	return Engines::findGameID(gameId, _gameIds, obsoleteGameIDsTable);
}

const char *CineMetaEngine::getName() const {
		return "Cinematique evo 1";
}

const char *CineMetaEngine::getOriginalCopyright() const {
...
bool CineEngine::hasFeature(EngineFeature f) const {
	return
		(f == kSupportsRTL) ||
