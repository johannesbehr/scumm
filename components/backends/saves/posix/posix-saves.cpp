...
#if 1//defined(POSIX) && !defined(DISABLE_DEFAULT_SAVEFILEMANAGER)
...
POSIXSaveFileManager::POSIXSaveFileManager() {
	...
	if (home && *home && strlen(home) < POSIX_MAXPATHLEN) {
		savePath = home;
	...
	if (!savePath.empty() && savePath.size() < POSIX_MAXPATHLEN) {
		ConfMan.registerDefault("savepath", savePath);
	}
#endif
	...
	if (!ConfMan.hasKey("savepath", Common::ConfigManager::kTransientDomain)) {
		...
		if (dir && *dir && strlen(dir) < POSIX_MAXPATHLEN) {
