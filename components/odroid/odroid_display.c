    ...
    else
    {
        ...
        if (scale)
        {
            ...
            for (y = 0; y < GAMEBOY_HEIGHT; y += 3)
            {
				...
				// Merge overlay into line_buffer
				if(overlay.visible){
					int left = overlay.x - 26;
					if((row<=overlay.x+overlay.height)&&(row+5>=overlay.x)){
						for(int i=0; i<5;i++){
							if((row+i>=overlay.x)&&(row+i<=overlay.x+overlay.height)){
								for(int j=0;j<overlay.width;j++){
									uint16_t pixel = overlay.pixel_data[(i + row - overlay.x) *overlay.width+j];
									line_buffer[i*outputWidth + j + left] = ((pixel >> 8) | ((pixel) << 8));
								}
							}
						}
					}
...
//return;
    ...
    else
    {
     //   uint16_t* framePtr = buffer;
            ...
            send_reset_drawing(0,0,320,height);

            for (y = 0; y < height; y += LINE_COUNT)
            {
              uint16_t* line_buffer = line_buffer_get();
...
//		send_continue_line(buffer, width, height);
    }
